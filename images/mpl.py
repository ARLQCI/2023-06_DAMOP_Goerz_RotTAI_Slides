...
def set_axis(
    ...
    start: float
        ...
        given. If not given, the current axis lower limit.
    stop: float
        value for last tick on the axis (and stop of axis, unless range is
        given). If not given, the current axis upper limit.
    step: float
        step between major ticks. If not given, use automatic ticks. Must not
        be given if logscale=True
    ticks: list
        If given, a list of major tick positions. Overrides start/stop/step.
    range: tuple
        The minimum and maximum value of the axis. If not given, [start, stop]
    bounds: tuple
        If given, the minimum and maximum values between which to draw the
        axsis (Tufte style)
    position: The position at which to draw the axes spine. Defaults to
        `('outward', 0)`. Use 'center' or 'zero' for a centered style
    show_opposite: Whether to show the opposite axis (right/top). Defaults to
        True
    minor:
    ...
    tick_params: dict
        ...
        `Axes.tick_params(axis="x/y", which="major", ...)`.
    minor_tick_params: dict
        Dictionary of arguments to
        `Axes.tick_params(axis="x/y", which="minor", ...)`.
    ...
    if which_axis == 'x':
        ...
        if bounds is not None:
            ax.spines['bottom'].set_bounds(bounds[0], bounds[1])
        ax.spines['bottom'].set_position(position)
        if not show_opposite:
        ...
        if logscale:
            ax.set_yscale('log')
        if bounds is not None:
            ax.spines['left'].set_bounds(bounds[0], bounds[1])
        ax.spines['left'].set_position(position)
        if not show_opposite:
    ...
    ax.tick_params(axis=which_axis, which="major", **tick_params)
    ax.tick_params(axis=which_axis, which="minor", **minor_tick_params)
